...
An access list must be passed along with the relay message tx. There are two admin RPC methods that can be used to construct the access list: `admin_getAccessListForIdentifier` and `admin_getAccessListByMsgHash`.

a. To get the access list using the `admin_getAccessListForIdentifier` RPC method, call the method with the identifier and the message payload.
...
b. To get the access list using the `admin_getAccessListByMsgHash` RPC method, call the method with the message hash. 
