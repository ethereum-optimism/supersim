...
contract DeployL2AuxilliaryContracts is Script {
    ...
    function deployUniswapV4() public {
        ...
        V4Router router = deployUniswapV4Router(address(stateView));
    ...
    function deployUniswapV4PositionManager(address _manager, address _positionsDescriptor) internal returns (PositionManager) {
        bytes memory args = abi.encode(_manager, _permit2, 1000000, _positionsDescriptor, Predeploys.WETH);
    ...
    /// @notice We use a MockV4Router since it exposes the needed routing functionality.
    function deployUniswapV4Router(address _manager) internal returns (V4Router) {
        bytes memory args = abi.encode(_manager);
        bytes memory initcode = abi.encodePacked(type(MockV4Router).creationCode, args);
        address addr = _deployer.computeAddress(_salt, keccak256(initcode));
        _deployer.deploy(0, _salt, initcode);
