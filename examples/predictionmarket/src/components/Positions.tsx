...
const Position: React.FC<{ market: Market }> = ({ market }) => {
    ...
    const { positions, redeem } = useMarketPositions(market);
        ...
        return (
            <div style={styles.positionCard}>
                <div style={styles.positionRow}>
                    ...
                    <div style={{...styles.cell, gap: '4px'}}>
                        <span>{(amount / 10 ** 18).toFixed(2)} ETH</span>
                        <span style={{fontStyle: 'italic',fontWeight: '600', color: '#0DA529'}}>({(payout/10 ** 18).toFixed(2)} ETH)</span>
                    </div>

                    <div style={{...styles.cell, display: 'flex', justifyContent: 'center'}}>
                        {
                            isLive ? <span style={{ fontStyle: 'italic', color: '#6b7280' }}>-</span> :
                            redeemable ?  <button style={styles.redeemButton} onClick={() => redeem(market.resolver, hasLP)}>Redeem</button> :
                            <span style={{ fontStyle: 'italic', color: '#6b7280' }}>Lost</span> 
                        }
                    </div>
                </div>
            </div>
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.tableHeader}>
                <div style={{...styles.headerCell, flex: 0.5}}>Status</div>
                <div style={styles.headerCell}>Chain</div>
                <div style={styles.headerCell}>Market</div>
                <div style={styles.headerCell}>Outcome</div>
                <div style={styles.headerCell}>Bet <span style={{fontSize: '14px', fontStyle: 'italic'}}>(incl LP)</span></div>
