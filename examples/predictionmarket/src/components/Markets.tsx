...
const MarketCard: React.FC<{ market: Market }> = ({ market }) => {
    const [isModalOpen, setIsModalOpen] = useState<{open: boolean, isYes: boolean}>({open: false, isYes: false});

    const { chainId, data, isResolvable, resolveMarket, isPending, isConfirming } = useMarketStatus(market)
    ...
    return (
        <div style={styles.marketCard}>
            <div style={styles.marketRow}>
                ...
                <div style={styles.cell}>
                    {isLive ?
                        <ProgressBar width='64px' height='32px' progress={yesOdds} yesColor='#0DA529' noColor='#0DA529' /> :
                        <span style={{marginLeft: '24px'}}>-</span>
                    }
                </div>

                <div style={styles.cell}>{(Number(market.ethBalance) / 10 ** 18).toFixed(2)} ETH</div>

                <div style={{...styles.cell, ...styles.statusText}}>
                    {
                        isResolvable ? <span>Live... (<span onClick={() => resolveMarket()} style={{textDecoration: 'underline', cursor: 'pointer'}}>resolve</span>)</span> :
                ...
                <div style={{...styles.cell, display: 'flex', justifyContent: 'center'}}>
                    <div style={styles.outcomeButtons}>
                        <button
                            ...
                            disabled={!isLive}
                            onClick={() => setIsModalOpen({open: true, isYes: true})}>
                            {yesText}
                        </button>
                        <button
                            style={{...styles.noButton, opacity: !isLive ? 0.5 : 1, cursor: !isLive ? 'not-allowed' : 'pointer'}}
                            disabled={!isLive}
                            onClick={() => setIsModalOpen({open: true, isYes: false})}>
                            {noText}
                        </button>
                    </div>
                </div>
            ...
            <MarketBetModal
                ...
                chainId={chainId}
                data={data}
                outcome={isModalOpen.isYes ? 1 : 2}
                yesOdds={yesOdds}
                yesText={yesText}
                noText={noText}
                market={market}
                onClose={() => setIsModalOpen({open: false, isYes: false})} />
        </div>
    )
}
...
const Markets: React.FC<{ markets: any[] }> = ({ markets }) => {
    ...
    return (
        <div style={styles.container}>
            ...
            {
                /* Market List (TODO: "No Markets" styling)*/
