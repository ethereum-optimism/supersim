...
const MarketCreateModal: React.FC<MarketCreateModalProps> = ({ isOpen, onClose }) => {
        ...
        <div style={styles.overlay}>
            <div style={styles.modal}>
                <div style={styles.header}>
                    <div style={{...styles.title, borderRight: '1px solid #E0E0E0'}}>Create Market</div>
                    <button style={styles.closeButton} onClick={() => {onClose(); setActiveTab(null)}}>×</button>
                </div>
...
const BlockHashModal: React.FC<{}> = () => {
    ...
    if (!latestBlockNumber) {
        return (<div style={{alignItems: 'center', justifyContent: 'center', display: 'flex'}}>Unable to fetch latest block</div>)
    }
        ...
        <>
            <div style={styles.field}>
                ...
                <select style={styles.select} value={selectedChain} onChange={(e) => setSelectedChain(Number(e.target.value))}>
                    <option value={901}>{chainName(901)}</option>
                    <option value={902}>{chainName(902)}</option>
                </select>
            </div>
            <div style={styles.field}>
