...
export const useMarketPositions = (market: Market) => {
    ...
    useEffect(() => {
        ...
        const fetchBalances = async () => {
            ...
            const liquidityProvided = await publicClient.getLogs({
                address: PREDICTION_MARKET_ADDRESS,
                event: parseAbiItem(["event LiquidityAdded(address indexed resolver, address indexed provider, uint256 ethAmountIn)"]),
                args: { resolver: market.resolver, provider: address },
                fromBlock: 'earliest', toBlock: 'latest'
            })
            ...
            const lpTotalEth = liquidityProvided.reduce((acc: number, bet: any) => acc + Number(bet.args.ethAmountIn), 0);

            // Outcome Amounts Received
            const yesTotalAmount = yesBets.reduce((acc: number, bet: any) => acc + Number(bet.args.amountOut), 0);
            const noTotalAmount = noBets.reduce((acc: number, bet: any) => acc + Number(bet.args.amountOut), 0);
