...
const useTicTacToeMarketStatus = (market: Market) => {
    ...
    return { chainId, data, isResolvable, resolveMarket, isPending, isConfirming }
}
...
const useBlockHashMarketStatus = (market: Market) => {
    ...
    const isResolvable = market.outcome === 0 && blockHashEvent !== null
    const data: BlockHashMarketStatus = { targetBlockNumber: targetBlockNumber ?? 0n }
    return { chainId: chainId ?? 0n, data, isResolvable, resolveMarket, isPending, isConfirming }
}